...
def check_3_2_trick(circuit, n, a, b, c):
...
def check(reply, n):
    circuit = read_circuit(reply)
    tests = (
        [([1]*n,[1]*n,[1]*n), ([0]*n,[0]*n,[0]*n)] 
        + 
        [tuple(randbinary(n) for i in [1,2,3]) for j in range(n)]
    )
    size, depth = get_size(circuit), get_depth(circuit)
    if not (size <= 20*n + 1 and depth <= 10):
        return False
    
    return all(
        check_3_2_trick(circuit, n, a, b, c) 
        for (a,b,c) in tests
    )